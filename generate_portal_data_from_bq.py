...
def fetch_videos(client, project_id: str, dataset: str, limit: int) -> list[dict]:
    ...
    SELECT
      ...
      COALESCE(c.group_role, '') AS group_role,
      COALESCE(c.group_parent_channel_id, '') AS group_parent_channel_id,
      COALESCE(c.genre_tag, '') AS genre_tag,
...
def make_video_record(row: dict) -> dict:
    ...
    return {
        ...
        "group_parent_channel_id": row.get("group_parent_channel_id") or "",
        "genre_tag": row.get("genre_tag") or "",
